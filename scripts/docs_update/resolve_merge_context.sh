...
default_branch="$(jq -r '.pull_request.base.repo.default_branch // .repository.default_branch // ""' "$GITHUB_EVENT_PATH")"
...
if [ "$action" != "closed" ]; then
  ...
  request_body="$(
    cat <<EOF
${default_branch} にマージされた PR を契機に、docs 更新要否を確認してください。
...
マージ済み PR の差分とドキュメントを読み、ドキュメント更新が必要な場合のみ docs 更新用ブランチを作成して新規 PR を作ってください。

ドキュメント更新にあたっては、以下の点に厳重に注意してください：
- ファイルパスやドキュメント内容に、個人の環境固有の絶対パス（例: /Users/username/...）や個人名を含めないこと。
- リポジトリルートからの相対パスを使用するか、汎用的なプレースホルダーを使用すること。
EOF
  )"
fi
